...
public class Lapras extends Creature
{
  /**
    ...
    public void attack(int idx)
    {
        ...
        else
        {
            ...
            if( enemyType.equalsIgnoreCase("Fire") )
            {
                ...
                getWorld().showText(" This attack was super effective! ", getWorld().getHeight()/2+26, getWorld().getWidth()/2);
                Greenfoot.delay(30);
            }
            else if( enemyType.equalsIgnoreCase("Rock") )
            {
              enemy.getHealthBar().add( -100*2 );
              getWorld().showText("This attack was super very effective!", getWorld().getHeight()/2+26, getWorld().getWidth()/2  );
              Greenfoot.delay(30);
            }
            else if ( enemyType.equalsIgnoreCase("Grass") )
            {
              enemy.getHealthBar().add( -100/2 );
              getWorld().showText("This attack not very effective!", getWorld().getHeight()/2+26, getWorld().getWidth()/2 ); 
              Greenfoot.delay(30);
            }
            else
            {
    ...
    /**
     * attackAnimation animates when the current creature attacks the other players current creature
    ...
    /**
     * switchCreature switches the current creature when the creature faints or the user just wants to switch the creature
     * @param their is an int parameter called idx
    ...
    /**
     * switchedIn switches out the current creature from the world into a creature that is not in the world
     * @param Their are no parameters
     * @return Nothing is returned
     */
